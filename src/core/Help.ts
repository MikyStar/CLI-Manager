...
class Help implements ManEntries {
	...
	constructor() {
		...
		this.resettingTask = {
			...
			prototype: "task r <task(s)> [-r] [global args]",
			argDef: [
				"<task(s)> : The id of the task you want to change, you can pass multiple by separating the ids by ',' without space",
				"-r: Also update target task's subtasks, 'recursive'",
			],
			furtherDescription: [
				"Will put tasks to the first state (first index in config file)",
			],
			globalArgs: true,
		};
	...
	getMan = (action: ManEntryKey) =>
		this.makeMan({ ...this[action], footer: true });
	...
	handleAction = (action: Action) => {
		...
		switch (action) {
			...
			case Action.CHECK:
				...
				break;
			case Action.RESET:
				toReturn = this.getMan("resettingTask");
				break;
