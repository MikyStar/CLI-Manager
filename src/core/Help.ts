...
class Help implements ManEntries {
  ...
  constructor() {
    ...
    this.globalArgs = [
      ...
      '',
      `--storage <relative path> : The specific storage file to use if path different than default ${bold(
      ...
      `--no-print : Prevent printing your tasks after having ran your command ; ${bold(
        "'shouldNotPrintAfter' in the config file",
      )}`,
    ...
    this.createStorage = {
      ...
      argDef: [`<relative path> : If you want to change the path, default ${bold(DEFAULT_STORAGE_FILE_NAME)}`],
    };
    ...
    this.createConfig = {
      ...
      furtherDescription: [
        ...
        `You don't need to have a configuration file, it's used to pass default ${bold(
          'global arguments',
    ...
    this.checkingTask = {
      ...
      furtherDescription: ['Will put tasks to the final state (last index in config file)'],
      globalArgs: true,
    };
    ...
    this.incrementingTask = {
      ...
      furtherDescription: ['Will put tasks to the next state (next index in config file)'],
      globalArgs: true,
    };
  ...
  fullMan = () => {
    ...
    return [...toReturn, ...this.globalArgs, ...this.footer];
  };
  ...
  private makeMan = (manPage: ManPage) => {
    ...
    if (manPage.title) toReturn = [...toReturn, underline(manPage.title), ''];

    if (manPage.prototype) toReturn = [...toReturn, bold(manPage.prototype), ''];

    if (manPage.argDef) toReturn = [...toReturn, ...manPage.argDef];

    if (manPage.furtherDescription) toReturn = [...toReturn, '', ...manPage.furtherDescription];

    if (manPage.examples) toReturn = [...toReturn, '', underline('Examples :'), '', ...manPage.examples];

    if (manPage.globalArgs) toReturn = [...toReturn, ...this.globalArgs];
