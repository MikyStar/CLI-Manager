...
export class Task implements ITask {
  ...
  stringify = (availableStates: TaskState[], options?: StringifyArgs) => {
    ...
    const {
      parentIndent,
      subTaskLevel = DEFAULT_SUBTASK_LEVEL,
      isSubTask,
      isLastChild,
      hideDescription,
      depth,
    ...
    if (isSubTask) {
      if (hideTree) indentation += INDENT_MARKER;
      else {
        ...
        if (isLastParent) indentation = indentation.split(TREE_CHARS.lastNode).join(INDENT_MARKER);
    ...
    const getPriorityText = () => {
      ...
      for (let i = 0; i < this.priority; i++) text += '!';
      ...
      const parseDescriptionLines = () => {
        ...
        descExploded.forEach((line) => {
          ...
          const handleChangeNodeIconToBranch = () => {
            ...
            if (!hideTree && toReplace && withWhat) toReturn = indentation.split(toReplace).join(withWhat);
    ...
    if (!this.subtasks || this.subtasks.length === 0) return toReturn;
    else {
