...
export class CliArgHandler {
  ...
  private handleOnOffFlags = (flag: OnOffFlag): boolean | undefined => {
    ...
    const occurances = this.extractOccurances(
      (arg) => arg.type === 'flag' && (arg.flagType === flag || arg.flagType === antiFlag),
    );
  ...
  private extractOccurances = (callback: (arg: RawArg, index?: number) => boolean): ArgOccurance[] | undefined => {
    ...
    for (let i = occurances.length - 1; i >= 0; i--) this.untreatedArgs.splice(occurances[i].index, 1);
  ...
  private rawParse = (args: string[]): RawArg[] => {
    ...
    for (let i = 0; i < args.length; i++) {
      ...
      if (theArg.includes(',') && !containsSpace) {
        ...
        if (isTask) parsedArgs.push({ value: Number.parseInt(theArg), type: 'task' });
        else if (isAction) parsedArgs.push({ value: theArg, type: 'action' });
        else if (isBooleanFlag) parsedArgs.push({ value: true, type: 'flag', flagType: theArg as BooleanFlag });
        else if (isOnOffFlag) parsedArgs.push({ value: true, type: 'flag', flagType: theArg as OnOffFlag });
        else if (isValueFlag) {
          ...
          else value = isNumber(followingValue) ? Number.parseInt(followingValue) : followingValue;

          parsedArgs.push({ value, type: 'flag', flagType: theArg as ValueFlag });
          i++; // Because we used the next value
        } else if (isPriority) parsedArgs.push({ value: theArg.length, type: 'priority' });
  ...
  private handleMultipleValuesType = (multipleArg: string) => {
    ...
    subParsed.forEach((arg) => {
      ...
      else if (arg.type !== argType) throw new MultipleValuesMismatchError(argType, arg.type);
    });
  ...
  private getBoolFlag = (flag: BooleanFlag): boolean | undefined => {
    const occurances = this.extractOccurances(
      (arg) => arg.type === 'flag' && arg.flagType === flag && arg.value === true,
    );
  ...
  private getValueFlag = (flag: ValueFlag) => {
    const occurances = this.extractOccurances(
      (arg) => arg.type === 'flag' && arg.flagType === flag && arg.value !== undefined,
    );
  ...
  private getPriority = (): number => {
    const occurances = this.extractOccurances((arg) => arg.type === 'priority' && arg.value !== undefined);
