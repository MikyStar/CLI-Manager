...
export class CliArgHandler {
	...
	constructor() {
		...
		this.flags = {
			...
			storageLocation: this.getValueFlag(
				ValueFlag.STORAGE_FILE,
			) as string,
	...
	private getPrinterConfig = (): PrinterConfig => {
		...
		const hideDescription = this.handleOnOffFlags(
			OnOffFlag.HIDE_DESCRIPTION,
		);
		const hideCompleted = this.handleOnOffFlags(OnOffFlag.HIDE_COMPLETED);
		const shouldNotPrintAfter = this.handleOnOffFlags(
			OnOffFlag.DONT_PRINT_AFTER,
		);
	...
	private handleOnOffFlags = (flag: OnOffFlag): boolean | undefined => {
		const orderedMap: OnOffFlag[] = [
			OnOffFlag.HIDE_DESCRIPTION,
			OnOffFlag.SHOW_DESCRIPTION,
			OnOffFlag.HIDE_COMPLETED,
			OnOffFlag.SHOW_COMPLETED,
			OnOffFlag.DONT_PRINT_AFTER,
			OnOffFlag.DO_PRINT_AFTER,
			OnOffFlag.CLEAR_BEFORE,
			OnOffFlag.DONT_CLEAR_BEFORE,
		];
		...
		const occurances = this.extractOccurances(
			(arg) =>
				arg.type === "flag" &&
				(arg.flagType === flag || arg.flagType === antiFlag),
		);

		if (occurances === undefined) return undefined;
	 ...
	 */
	private extractOccurances = (
		callback: (arg: RawArg, index?: number) => boolean,
	): ArgOccurance[] | undefined => {
		const occurances: ArgOccurance[] = [];

		this.untreatedArgs.forEach((arg, index) => {
			if (callback(arg, index)) occurances.push({ arg, index });
		});

		if (occurances.length === 0) return undefined;
	...
	private rawParse = (args: string[]): RawArg[] => {
		...
		for (let i = 0; i < args.length; i++) {
			...
			if (theArg.includes(",") && !containsSpace) {
				const { subParsed, argType } =
					this.handleMultipleValuesType(theArg);

				const values = subParsed.map((sub) => sub.value) as
					| string[]
					| number[];
				parsedArgs.push({ value: values, type: argType });
			} else {
				const isTask = isNumber(theArg);
				const isAction = Object.values(Action).includes(
					theArg as Action,
				);
				const isBooleanFlag = Object.values(BooleanFlag).includes(
					theArg as BooleanFlag,
				);
				const isOnOffFlag = Object.values(OnOffFlag).includes(
					theArg as OnOffFlag,
				);
				const isValueFlag = Object.values(ValueFlag).includes(
					theArg as ValueFlag,
				);
				...
				if (isTask)
					...
					parsedArgs.push({
						value: true,
						type: "flag",
						flagType: theArg as BooleanFlag,
					});
				else if (isOnOffFlag)
					parsedArgs.push({
						value: true,
						type: "flag",
						flagType: theArg as OnOffFlag,
					});
				else if (isValueFlag) {
					...
					if (
						isGroupBy &&
						!handledGroupings.includes(
							followingValue as GroupByType,
						)
					)
						throw new GroupBySyntaxError(
							`'--group' following attribute should be from '${handledGroupings.map((str) => str)}'`,
						);
					else
						value = isNumber(followingValue)
							? Number.parseInt(followingValue)
							: followingValue;

					parsedArgs.push({
						value,
						type: "flag",
						flagType: theArg as ValueFlag,
					});
					i++; // Because we used the next value
				} else if (isPriority)
					parsedArgs.push({ value: theArg.length, type: "priority" });
				else parsedArgs.push({ value: theArg, type: "text" });
			}
		}
	...
	private handleMultipleValuesType = (multipleArg: string) => {
		...
		subParsed.forEach((arg) => {
			if (argType === undefined) argType = arg.type;
	...
	private getPriority = (): number => {
		const occurances = this.extractOccurances(
			(arg) => arg.type === "priority" && arg.value !== undefined,
		);
